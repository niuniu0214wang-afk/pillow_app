...
- (NSInteger)numberWithHexString:(NSString *)hexString{
    ...
    NSString *string = [NSString stringWithFormat:@"0x%@",hexString];
...
#pragma mark - 筛选字符串并NSdata转字符串
- (NSString *)hexDataToString:(NSString *)userStr
{
    NSLog(@"传进来的数据是什么-------%@",userStr);
    NSMutableString *string = [NSMutableString stringWithString:@""];
    for (int i = 0; i < 8; i++) {
        NSString *str = [userStr substringWithRange:NSMakeRange(i*2, 2)];
        NSLog(@"每次截取的数据是什么------%@",str);
        if (![str isEqualToString:@"ff"]) {
            [string appendString:str];
        }
    }
    NSLog(@"-------string is %@",string);
    NSString *strHex = [string stringByReplacingOccurrencesOfString:@" " withString:@""];
    NSLog(@"========string is %@",strHex);
    NSData *data = [self convertHexStrToData:strHex];
    NSString *nickName = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    NSLog(@"解析出来的昵称是------%@",nickName);
    return nickName;
}
