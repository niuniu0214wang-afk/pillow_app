...
- (CGFloat)setupLineAndVerticalLabel {
    ...
    float heightGap = (self.frame.size.height - maxHeight - VerticalLineGap) / 5.0;
    
    int highestValueEColumnChart = [self maxValueForCharDataModels].value;
    int valueGap = highestValueEColumnChart / 5.0;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        CGRect frame = CGRectMake(10, -heightGap / 2.0 + heightGap * (4 - i), textWidth, heightGap);
        ...
        eColumnChartLabel.text = [NSString stringWithFormat:@"%d", number];
...
- (CGFloat)dataModelsMaxHeight {
    ...
    CGFloat maxHeight = [[array valueForKeyPath:@"@max.self.intValue"] floatValue];
    return maxHeight;
}
...
- (void)logic {
    ...
    for (int i = 0; i < totalCount; i++) {
        ...
        bar.barColor = self.barNormalColor;
        if (i == totalCount - 1) {
            bar.barColor = [UIColor colorWithValue:@"#45d276"];
        }
...
{    
    PGBar *bar = (PGBar *)tapGesture.view;
    bar.barColor = [UIColor colorWithValue:@"#45d276"];
    if (tapGesture.view.tag != _currentIdx) {
        
        PGBar *lastBar = (PGBar *)[self viewWithTag:_currentIdx];
        lastBar.barColor = self.barNormalColor;
        _currentIdx = bar.tag;
    }
